...
## Generated declaration formatting

The type builders write each member of a named union on its own line in
`__generated__/` and `dist/`, so adding or removing a value produces a small diff.
This package-local generation step runs after TypeScript emits declarations;
TypeScript otherwise collapses union members onto one line. Inline property and
parameter annotations keep their existing layout.

Run `pnpm --filter=@unlayer/types build` to regenerate the declarations and
`pnpm --filter=@unlayer/types test:unit` to check the formatter. Do not manually
reformat generated files or change the repository-wide Prettier configuration.
